...
const employ_session = await onnxruntime.InferenceSession.create(
  './src/models/employed_model.onnx'
);
const salary_session = await onnxruntime.InferenceSession.create(
  './src/models/salary_model.onnx'
);

export const createInputs = (value_object) => {
  const features = {
  ...
  const inputs = Object.fromEntries(
    Object.entries(features).map(([name, value]) => [
      ...
      typeof value[0] === 'string'
        ? new onnxruntime.Tensor('string', value, [1, 1])
        : new onnxruntime.Tensor('float32', value, [1, 1]),
    ])
  );
